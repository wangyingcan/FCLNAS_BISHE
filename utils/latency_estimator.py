...
class LatencyEstimator(object):
    ...
    def predict(self, ltype: str, _input, output, expand=None, kernel=None, stride=None, idskip=None, ):
        ...
        key = '-'.join(infos)
        # 缺表时返回 0 并打印一次警告，避免 KeyError 中断训练
        try:
            return self.lut[key]
        except KeyError:
            if not hasattr(self, "_missing_warn"):
                self._missing_warn = set()
            if key not in self._missing_warn:
                # sys.stderr.write(f"[LatencyEstimator] missing key: {key}, fallback 0\n")
                self._missing_warn.add(key)
            return 0.0
